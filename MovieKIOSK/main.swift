...
while choice != "0" {
    ...
    case "1":
        ...
        let seats = movieList[movieList.firstIndex(where: {$0.title == title && $0.time == time})!]
        ...
        if readLine()! == "Y" {
            //잔고가 충분하면
            bookedList.append(Ticket(title: title, time: time, headCount: headCount, seats: pickedSeat, phoneNumber: phoneNumber))
            print("예매가 완료되었습니다")
        } else {
            print("결제가 취소되었습니다")
        }
